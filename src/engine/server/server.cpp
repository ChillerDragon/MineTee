...
int CServer::NewClientCallback(int ClientID, void *pUser)
{
	...
	pThis->m_aClientsMapInfo[ClientID].m_pCurrentMapData = 0x0; // MineTee
	return 0;
}
...
int CServer::DelClientCallback(int ClientID, const char *pReason, void *pUser)
{
	...
	// MineTee
    mem_free(pThis->m_aClientsMapInfo[ClientID].m_pCurrentMapData);
    //
	...
	if(Sys)
	{
		...
		if(Msg == NETMSG_INFO)
		{
			if((pPacket->m_Flags&NET_CHUNKFLAG_VITAL) != 0 && m_aClients[ClientID].m_State == CClient::STATE_AUTH)
			{
				...
				// MineTee
				const char *pClientName = Unpacker.GetString(CUnpacker::SANITIZE_CC);
				if(str_comp(pClientName, "MineTee") != 0)
				{
					// wrong client
		...
		else if(Msg == NETMSG_REQUEST_MAP_DATA)
		{
			...
			if(Chunk < 0 || Offset > m_aClientsMapInfo[ClientID].m_CurrentMapSize) // MineTee
				return;

			if(Offset+ChunkSize >= m_aClientsMapInfo[ClientID].m_CurrentMapSize) // MineTee
			{
				ChunkSize = m_aClientsMapInfo[ClientID].m_CurrentMapSize-Offset;
				if(ChunkSize < 0)
					ChunkSize = 0;
				Last = 1;
			}
			...
			Msg.AddRaw(&m_aClientsMapInfo[ClientID].m_pCurrentMapData[Offset], ChunkSize); // MineTee
			SendMsgEx(&Msg, MSGFLAG_VITAL|MSGFLAG_FLUSH, ClientID, true);
		...
		else if (Msg == NETMSG_CELL_MOVE)
		{
			...
			if((pPacket->m_Flags&NET_CHUNKFLAG_VITAL) != 0 && Unpacker.Error() == 0)
			{
				GameServer()->OnClientMoveCell(ClientID, From, To, Qty);
			}
		}
		else if (Msg == NETMSG_RELEASE)
		{
			GameServer()->OnClientRelease(ClientID);
		}
		//
		else
		{
...
void CServer::SendServerInfo(const NETADDR *pAddr, int Token)
{
	...
	str_format(aBuf, sizeof(aBuf), "%d", m_NetServer.MaxClients()-g_Config.m_SvSpectatorSlots-MAX_BOTS); p.AddString(aBuf, 3); // max players
...
void CServer::ConGive(IConsole::IResult *pResult, void *pUser)
{
	if (pResult->NumArguments() > 2)
		((CServer *)pUser)->m_pGameServer->GiveItem(pResult->GetInteger(0), pResult->GetInteger(1), pResult->GetInteger(2));
}
void CServer::ConMapSave(IConsole::IResult *pResult, void *pUser)
{
	if (pResult->NumArguments() == 1)
		((CServer *)pUser)->m_pGameServer->SaveMap(pResult->GetString(0));
	else
		((CServer *)pUser)->m_pGameServer->SaveMap("");
}
