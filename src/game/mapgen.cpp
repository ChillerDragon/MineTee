...
void CMapGen::GenerateBasicTerrain()
{
	...
	for(int i = 0; i < GameServer()->Layers()->MineTeeLayer()->m_Width; i++)
	{
		...
		{
			GameServer()->Collision()->ModifTile(vec2(TilePosX, TempTileY), GameServer()->Layers()->GetMineTeeGroupIndex(), GameServer()->Layers()->GetMineTeeLayerIndex(), CBlockManager::DIRT, 0);
			TempTileY++;
		}
	}
	...
	for(int i = 0; i < GameServer()->Layers()->MineTeeLayer()->m_Width; i++)
	{
		...
		GameServer()->Collision()->ModifTile(vec2(TilePosX, TilePosY), GameServer()->Layers()->GetMineTeeGroupIndex(), GameServer()->Layers()->GetMineTeeLayerIndex(), CBlockManager::STONE, 0);
		
		// fill the tiles under the random tile
		int TempTileY = TilePosY+1;
		while(TempTileY < GameServer()->Layers()->MineTeeLayer()->m_Height)
		{
			GameServer()->Collision()->ModifTile(vec2(TilePosX, TempTileY), GameServer()->Layers()->GetMineTeeGroupIndex(), GameServer()->Layers()->GetMineTeeLayerIndex(), CBlockManager::STONE, 0);
			TempTileY++;
		}
	}
}
	...
	for(int x = 1; x < GameServer()->Layers()->MineTeeLayer()->m_Width-1; x++)
	{
		if(rand()%32 == 0)
		{
			...
			for(int f = x; f - x <= FieldSize; f++)
			{
				...
				GameServer()->Collision()->ModifTile(vec2(f, y), GameServer()->Layers()->GetMineTeeGroupIndex(), GameServer()->Layers()->GetMineTeeLayerIndex(), CBlockManager::ROSE + rand()%2, 0);
			}
	...
	for(int x = 1; x < GameServer()->Layers()->MineTeeLayer()->m_Width-1; x++)
	{
		if(rand()%256 == 0)
		{
			...
			for(int f = x; f - x <= FieldSize; f++)
			{
				...
				GameServer()->Collision()->ModifTile(vec2(f, y), GameServer()->Layers()->GetMineTeeGroupIndex(), GameServer()->Layers()->GetMineTeeLayerIndex(), CBlockManager::MUSHROOM_RED + rand()%2, 0);
			}
	...
	for(int i = 10; i < GameServer()->Layers()->MineTeeLayer()->m_Width-10; i++)
	{
		...
		if((rand()%64 == 0 && abs(LastTreeX - i) >= 8) || (abs(LastTreeX - i) <= 8 && abs(LastTreeX - i) >= 3 && rand()%8 == 0))
		{
			...
			for(int h = 0; h <= TreeHeight; h++)
				GameServer()->Collision()->ModifTile(vec2(i, TempTileY-h), GameServer()->Layers()->GetMineTeeGroupIndex(), GameServer()->Layers()->GetMineTeeLayerIndex(), CBlockManager::WOOD_BROWN, 0);
		
			// add the leafs
			for(int l = 0; l <= TreeHeight/2.5f; l++)
				GameServer()->Collision()->ModifTile(vec2(i, TempTileY - TreeHeight - l), GameServer()->Layers()->GetMineTeeGroupIndex(), GameServer()->Layers()->GetMineTeeLayerIndex(), CBlockManager::LEAFS, 0);
			...
			for(int h = 0; h <= TreeHeight/2; h++)
			{
				for(int w = 0; w < TreeWidth; w++)
				{
					...
					GameServer()->Collision()->ModifTile(vec2(i-(w-(TreeWidth/2)), TempTileY-(TreeHeight-(TreeHeight/2.5f)+h)), GameServer()->Layers()->GetMineTeeGroupIndex(), GameServer()->Layers()->GetMineTeeLayerIndex(), CBlockManager::LEAFS, 0);
				}
			}
...
void CMapGen::GenerateBorder()
{
	...
	for(int i = 0; i < GameServer()->Layers()->MineTeeLayer()->m_Width; i++)
	{
		...
		// bottom border
		GameServer()->Collision()->ModifTile(vec2(i, GameServer()->Layers()->MineTeeLayer()->m_Height-1), GameServer()->Layers()->GetMineTeeGroupIndex(), GameServer()->Layers()->GetMineTeeLayerIndex(), CBlockManager::BEDROCK, 0);
	}

	for(int i = 0; i < GameServer()->Layers()->MineTeeLayer()->m_Height; i++)
	{
		// left border
		GameServer()->Collision()->ModifTile(vec2(0, i), GameServer()->Layers()->GetMineTeeGroupIndex(), GameServer()->Layers()->GetMineTeeLayerIndex(), CBlockManager::BEDROCK, 0);
		
		// right border
		GameServer()->Collision()->ModifTile(vec2(GameServer()->Layers()->MineTeeLayer()->m_Width-1, i), GameServer()->Layers()->GetMineTeeGroupIndex(), GameServer()->Layers()->GetMineTeeLayerIndex(), CBlockManager::BEDROCK, 0);
	}
}
