...
void CEffects::OnRender()
{
	...
	static int64 LastUpdate10hz = 0; // MineTee
	...
	if(Client()->State() == IClient::STATE_DEMOPLAYBACK)
	{
		...
		// MineTee
		if(time_get()-LastUpdate10hz > time_freq()/(10*pInfo->m_Speed))
		{
			m_Add10hz = true;
			LastUpdate10hz = time_get();
		}
		else
			m_Add10hz = false;
		//
	...
	// MineTee
	if(time_get()-LastUpdate10hz > time_freq()/10)
	{
		m_Add10hz = true;
		LastUpdate10hz = time_get();
	}
