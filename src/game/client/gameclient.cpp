...
void CGameClient::OnConnected()
{
	...
	m_PaintMode = PAINT_NORMAL; // MineTee
...
void CGameClient::OnNewSnapshot()
{
	...
	// go trough all the items in the snapshot and gather the info we want
	{
		...
		for(int i = 0; i < Num; i++)
		{
			...
			if(Item.m_Type == NETOBJTYPE_CLIENTINFO)
			{
				...
				// prepare the info
				if(!str_find_nocase(CurrentServerInfo.m_aGameType, "minetee") && (m_aClients[ClientID].m_aSkinName[0] == 'x' || m_aClients[ClientID].m_aSkinName[1] == '_')) // MineTee
			...
			else if(Item.m_Type == NETOBJTYPE_PLAYERINFO)
			{
				...
				if(pInfo->m_Team != TEAM_SPECTATORS && pInfo->m_Team <= TEAM_BLUE) // MineTee
	...
	int Teams[4] = { TEAM_RED, TEAM_BLUE, TEAM_SPECTATORS, TEAM_BOT };
	int Index = 0;
...
{
	int IsCurrentBG = (((CGameClient *)pUserData)->m_PaintMode == PAINT_BACKGROUND);
	((CGameClient *)pUserData)->m_PaintMode = IsCurrentBG?PAINT_NORMAL:PAINT_BACKGROUND;
}

void CGameClient::ConForegroundPaint(IConsole::IResult *pResult, void *pUserData)
{
	int IsCurrentFG = (((CGameClient *)pUserData)->m_PaintMode == PAINT_FOREGROUND);
	((CGameClient *)pUserData)->m_PaintMode = IsCurrentFG?PAINT_NORMAL:PAINT_FOREGROUND;
}

void CGameClient::ConDropItem(IConsole::IResult *pResult, void *pUserData)
{
	int ItemID = -1;
